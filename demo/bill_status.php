...
$count_result = $conn->query("SELECT COUNT(*) as total FROM bills b JOIN patients p ON p.patient_id = b.patient_id $where_sql");
if (!$count_result) {
...
if (!$result) {
  die("Query Error: " . $conn->error . "<br><pre>$query</pre>");
}
?>
<!DOCTYPE html>
<html>
      ...
      <tr>
        ...
        <td>
          <?php if ($row['result_entered']): ?>
            <a href="report_template_selector.php?bill_id=<?= $row['bill_id'] ?>" target="_blank" class="print-button">Print Report</a>
          <?php else: ?>
