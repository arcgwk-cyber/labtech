...
// Ensure table has template_format column
$conn->query("
  CREATE TABLE IF NOT EXISTS patient_types (
    type_id INT AUTO_INCREMENT PRIMARY KEY,
    type_name VARCHAR(100) NOT NULL,
    description TEXT NULL,
    template_format TEXT NULL,
...
if (isset($_POST['add'])) {
    ...
    $tpl  = $_POST['template_format'] ?? '';
    if (!empty($name)) {
        $stmt = $conn->prepare("INSERT INTO patient_types (type_name, description, template_format) VALUES (?, ?, ?)");
        $stmt->bind_param("sss", $name, $desc, $tpl);
        $stmt->execute();
        $stmt->close();
    }
...
if (isset($_POST['update'])) {
    ...
    $desc = trim($_POST['description']);
    $tpl  = $_POST['template_format'] ?? '';
    $stmt = $conn->prepare("UPDATE patient_types SET type_name=?, description=?, template_format=? WHERE type_id=?");
    $stmt->bind_param("sssi", $name, $desc, $tpl, $id);
    $stmt->execute();
    $stmt->close();
    header("Location: patient_types.php");
    exit;
...
<head>
    ...
    <style>
        ...
        .template-preview { max-height: 150px; overflow: auto; border:1px solid #ddd; padding:5px; }
    </style>
</head>
<body>
<div class="container">
    ...
    <div class="card p-4 mb-4">
        <form method="POST">
            ...
            <div class="mb-3">
                ...
                <textarea name="template_format" id="add_template_format" class="form-control" rows="5"></textarea>
                <small class="text-muted">
                    Placeholders: 
                    <code>{{full_name}}, {{dob}}, {{age}}, {{gender}}, {{phone}}, {{address}}, {{bill_id}}, {{bill_date}}, {{field_X}}</code>
                </small>
            </div>
        ...
        <table class="table table-bordered table-hover">
            ...
            <?php foreach ($types as $i => $type): ?>
                <tr>
                    ...
                    <td><div class="template-preview"><?= $type['template_format'] ?></div></td>
                    <td>
                            ...
                            <form method="POST">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        ...
                                        <div class="mb-3">
                                            <label class="form-label">Description</label>
                                            <textarea name="description" class="form-control" rows="2"><?= htmlspecialchars($type['description']) ?></textarea>
                                        </div>
                                        <div class="mb-3">
                                            <label class="form-label">Template Format</label>
                                            <textarea name="template_format" id="edit_template_format<?= $type['type_id'] ?>" class="form-control" rows="6"><?= htmlspecialchars($type['template_format']) ?></textarea>
                                        </div>
                                    </div>
                        ...
                        <script>CKEDITOR.replace('edit_template_format<?= $type['type_id'] ?>');</script>
                    </td>
                </tr>
            <?php endforeach; ?>
            <?php if ($types->num_rows == 0): ?>
                <tr><td colspan="5" class="text-center">No patient types found.</td></tr>
            <?php endif; ?>
            </tbody>
        </table>
    </div>
...
<script> CKEDITOR.replace('add_template_format'); </script>
</body>
</html>
