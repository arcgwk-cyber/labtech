...
<div class="row mb-3">
    <div class="col-md-6">
        ...
        <select name="signature_id" id="signature_id" class="form-control" onchange="previewImage(this,'sig_preview')">
            ...
            <?php
            $res = $conn->query("SELECT id, name, role, qualification, signimage FROM sign_master WHERE signimage<>''");
            if ($res) {
                while($row = $res->fetch_assoc()):
                    $selected = ($test['signature_id'] == $row['id']) ? 'selected' : '';
                    $file = 'sign_stamp/'.$row['signimage'];
                    $label = $row['name'] . (!empty($row['role']) ? ' (' . $row['role'] . (!empty($row['qualification']) ? ' - ' . $row['qualification'] : '') . ')' : '');
            ?>
                    <option value="<?= $row['id'] ?>" <?= $selected ?> data-file="<?= $file ?>"><?= htmlspecialchars($label) ?></option>
            <?php 
                endwhile;
            }
            ?>
    ...
    <div class="col-md-6">
        ...
        <select name="stamp_id" id="stamp_id" class="form-control" onchange="previewImage(this,'stamp_preview')">
            ...
            <?php
            $res = $conn->query("SELECT id, name, role, qualification, stampimage FROM sign_master WHERE stampimage<>''");
            if ($res) {
                while($row = $res->fetch_assoc()):
