...
function appendTestNotesAndSignature($test_id, $qr_link, $pdf) {
    global $conn;
    ...
    $stmt->fetch();
    $stmt->close();
// Step 2: Remove <figure> and add border attributes to table and cells
$interpretation = preg_replace('/<figure[^>]*>/', '', $interpretation); // remove opening <figure>
$interpretation = str_replace('</figure>', '', $interpretation);         // remove closing </figure>

// Step 3: Add table border and styles
$interpretation = str_replace('<table', '<table border="1" cellpadding="4" cellspacing="0" style="border-collapse:collapse;"', $interpretation);
...
while ($row = $results->fetch_assoc()) {
    ...
    if (!in_array($combo_key, $printed_combos)) {
        if (!empty($html)) {
            $html .= "</table><br>";
            $pdf->writeHTML($html, true, false, true, false, '');
            appendTestNotesAndSignature($last_test_id, $qr_link, $pdf);
            $pdf->AddPage();
        }
