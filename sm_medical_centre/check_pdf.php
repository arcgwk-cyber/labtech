...
<table width="100%" cellpadding="2" cellspacing="0" style="font-family: Helvetica, Arial, sans-serif;">
    <tr>
        <td width="12%"><strong>Name</strong></td><td width="2%">:</td><td width="42%">{$bill['full_name']}</td>
        <td width="19%"><strong>Bill No.</strong></td><td width="2%">:</td><td width="20%">{$bill['bill_id']}</td>
    </tr>
    <tr>
        <td><strong>Age/Gender</strong></td><td>:</td><td>{$age} / {$gender}</td>
        <td><strong>Registered Date</strong></td><td>:</td><td>{$bill['bill_date']}</td>
    </tr>
    <tr>
        <td><strong>Referred By</strong></td><td>:</td><td>Dr.</td>
        <td><strong>Reported Date</strong></td><td>:</td><td>{$report_date}</td>
    </tr>
...
function getGroupHeader($groupName) {
    $groupName = strtoupper(htmlspecialchars($groupName));
    return <<<EOD
...
<tr style="background-color:#f2f2f2; font-weight:bold;">
    <td width="40%">TEST DESCRIPTION</td>
    <td width="15%">RESULT</td>
    <td width="15%">UNIT</td>
    <td width="30%">REFERENCE RANGE</td>
</tr>
EOD;
}
...
function getResultDisplay($row, $min, $max) {
    ...
    return $highlight ? '<span style="color:red; font-weight:bold;">' . htmlspecialchars($value) . '</span>' : htmlspecialchars($value);
}

function appendTestNotesAndSignature($test_id, $qr_link, $pdf, $include_notes, $include_interpretation) {
    global $conn;

    $stmt = $conn->prepare("SELECT notes, interpretations FROM lab_tests WHERE test_id = ?");
...
$pdf->SetMargins(10, 25.4, 10, true);
...
// --- Fetch Test Results ---
$stmt = $conn->prepare("
    SELECT g.group_name, t.test_id, t.test_name, ltp.param_order, tp.param_name, tp.unit, tp.method,
...
if ($results->num_rows == 0) die("No test results found.");
...
if ($pagebreak_per_test) {
    ...
    while ($row = $results->fetch_assoc()) {
        ...
        $test_key = "{$group_name}|{$test_name}";

        if (!in_array($test_key, $printed_tests)) {
            if (!empty($html)) {
                $html .= "</table><br>";
                $pdf->writeHTML($html, true, false, true, false, '');
            ...
            $html .= getTestHeader();
            $html .= "<tr><td colspan='4'><strong>" . htmlspecialchars($test_name) . "</strong></td></tr>";
            $printed_tests[] = $test_key;
        ...
        $html .= "<tr>
            <td style='border:1px solid #000; padding:5px;'>" . htmlspecialchars($row['param_name']) . "{$method}</td>
            <td style='border:1px solid #000; padding:5px;'>{$result_display}</td>
            <td style='border:1px solid #000; padding:5px;'>" . htmlspecialchars($row['unit']) . "</td>
            <td style='border:1px solid #000; padding:5px;'>" . htmlspecialchars($ref_range) . "</td>
        </tr>";
    }
    ...
    // ---- GROUPED FORMAT (existing version when pagebreak is false) ----
    $pdf->AddPage();
    $html = generatePatientHTML($bill, $age, $gender, $report_date);
